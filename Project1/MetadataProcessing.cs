...
namespace TranslationENMOD
{
    ...
    internal static class MetadataProcessing
    {
        ...
        static public void ProcessMetadata()
        {
            ...
            LibCpp2IL.LibCpp2IlMain.LoadFromFile(pepath, path, new int[] { 2020, 3, 12 });
            var metadata = LibCpp2IL.LibCpp2IlMain.TheMetadata;
            ...
            for (int i = 0; i < header.stringLiteralCount; i++)
            {
                try
                {
                    var str = LibCpp2IL.LibCpp2IlMain.TheMetadata.GetStringLiteralFromIndex((uint)i);
                    ...
                    if (Helpers.IsChinese(str))
                    {
                        ...
                        if (HasNoInvalidChars(finalstring))
                        {
                            ...
                            MetaDataReplace(finalstring, LibCpp2IL.LibCpp2IlMain.TheMetadata.GetStringLiteralFromIndex((uint)i));
            ...
            if (processbuffer.Count > 0)
            {
                ...
                LibCpp2IL.LibCpp2IlMain.LoadFromFile(pepath, path + ".temp", new int[] { 2020, 3, 12 });
                metadata = LibCpp2IL.LibCpp2IlMain.TheMetadata;
                Plugin.log.LogInfo("Here for now");
